(() => {
  ...
  function bindPage(){
    ...
    $('[data-change-username]')?.addEventListener('click',openUsernameModal);
  ...
  function usernameNextChangeDate(user){
    if(!user?.username_changed_at)return null;
    const next=new Date(user.username_changed_at);
  ...
  function openUsernameModal(){
    const me=current(),next=usernameNextChangeDate(me),blocked=next&&next.getTime()>Date.now();
    modal('Gebruikersnaam wijzigen',blocked
      ? `<div class="username-cooldown"><strong>@${esc(me.username)}</strong><p>Je kunt je gebruikersnaam opnieuw wijzigen vanaf <b>${esc(formatLongDate(next))}</b>.</p></div>`
      : `<form id="usernameForm"><label>Nieuwe gebruikersnaam<input name="username" value="${esc(me.username)}" autocapitalize="none" autocomplete="off" minlength="3" maxlength="30" pattern="[a-zA-Z0-9._-]{3,30}" required></label><p class="muted">Gebruik 3–30 tekens: letters, cijfers, punt, streepje of underscore. Na opslaan kun je dit pas over 6 maanden opnieuw wijzigen.</p><button class="btn primary full" type="submit">Gebruikersnaam opslaan</button></form>`,()=>{
        const form=$('#usernameForm');
        if(!form)return;
        form.onsubmit=e=>{e.preventDefault();const value=String(new FormData(form).get('username')||'').trim().toLowerCase();run(()=>DB.changeUsername(value),'Gebruikersnaam gewijzigd');};
      });
  }
