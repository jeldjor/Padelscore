(() => {
  ...
  function renderReserveList(reserves,users){
    ...
    return `<section class="reserve-list"><header><h3>Reservelijst</h3><span>${reserves.length} reserve${reserves.length===1?'':'s'}</span></header><div>${reserves.map((r,i)=>`<div class="signup-player-row"><b class="reserve-position">${i+1}</b><strong class="avatar-name">${esc(users.get(r.user_id)?.display_name||'Onbekend')}${avatarMarkup(users.get(r.user_id))}</strong><span class="mini-pill reserve">Plek ${i+1}</span></div>`).join('')}</div></section>`;
  }
  ...
  function openManualResult(id){
    ...
    modal('Uitslag invullen',`<form id="manualResultForm" class="manual-result-form"><div class="manual-team blue"><strong>${playerNameMarkup(m.blue_player_1)} & ${playerNameMarkup(m.blue_player_2)}</strong><input name="blue_games" type="number" inputmode="numeric" min="0" max="99" required placeholder="Games"></div><div class="manual-versus">—</div><div class="manual-team red"><strong>${playerNameMarkup(m.red_player_1)} & ${playerNameMarkup(m.red_player_2)}</strong><input name="red_games" type="number" inputmode="numeric" min="0" max="99" required placeholder="Games"></div><button class="btn primary full">Uitslag opslaan</button></form>`,()=>{$('#manualResultForm').onsubmit=e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.target));run(()=>DB.finishMatchManual(id,f.blue_games,f.red_games),'Uitslag opgeslagen');};});
  }
