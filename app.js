(() => {
  ...
  const state = { page:'dashboard', year:new Date().getFullYear(), month:new Date().getMonth(), playdayView:'calendar', playdayFilter:'all', playdayList:true, selectedPlaydayId:null, activeMatchId:null, scoreboardMatchId:null, wakeLock:null, timer:null, controlsTimer:null, recognition:null, adminTab:'requests', scoreboardReadOnly:false };
  ...
  function showApp(){ $('#loginScreen').classList.add('hidden'); $('#appShell').classList.remove('hidden'); $('#accountNavLabel').textContent=isAdmin()?'Beheer':'Account'; if(current()?.must_change_password) openPasswordModal(true); render(); }
  ...
  function render(){ const main=$('#mainContent'); if(!current()){showLogin();return;} const pages={dashboard:renderDashboard,playday:renderPlayday,ranking:renderRanking,history:renderHistory,account:renderAccount}; main.innerHTML=(pages[state.page]||renderDashboard)(); bindPage(); }
  ...
  function poweredFooter(){ return `<div class="page-powered"><span>POWERED BY</span><img src="logo-gjmotion.png" alt="GJ MOTION"></div>`; }
  ...
  function renderDashboard(){
    ...
    return `${uiHeader('Goedemiddag','Jouw persoonlijke competitie-overzicht van vandaag.')}
      <section class="dashboard-hero"><div class="dashboard-hero-name"><h2>${esc(me.display_name)}</h2></div></section>
  ...
  function renderCalendar(){
    ...
    return `${uiHeader('Speeldagen','Bekijk alle speeldagen per maand.')}
  ...
  function renderPlayday(){
    if(state.playdayList){
      ...
      return `${uiHeader('Speeldagen','Bekijk alle speeldagen, ook wanneer je zelf niet meedoet.',isAdmin()?'<button class="btn primary small" data-add-pd>+ Speeldag</button>':'')}
    ...
    const lobbyBox=lobbyAllowed?`<section class="flat-section"><div class="section-title"><div><h2>Lobby</h2><p class="muted">Eén keer READY voor de hele speeldag.</p></div>${at?`<span class="badge ${at.status==='ready'?'green':'yellow'}">${esc(statusLabel(at.status))}</span>`:''}</div>${!at?'<button class="btn primary full" data-enter-lobby>Lobby binnengaan</button>':at.status==='present'?'<button class="btn primary full" data-ready>Ik ben READY</button>':`<p class="muted">Je blijft ${esc(statusLabel(at.status))} tot de host je status verandert.</p>`}<div class="open-rows">${attendance.map(x=>`<div class="open-row"><span class="status-dot ${statusClass(x.status)}"></span><strong>${esc(x.user.display_name)}</strong><small>${esc(statusLabel(x.status))}</small></div>`).join('')||'<div class="empty-state">Nog niemand in de lobby.</div>'}</div></section>`:'';
    ...
    return `<button class="detail-back" data-back-list>‹</button>${uiHeader(esc(fmtDate(pd.date)), '', '')}
      ...
      <section class="flat-section"><div class="section-title"><h2>Aangemeld (${all.length})</h2><span class="host-pill">Host: ${esc(nameOf(pd.host_id))}</span></div><div class="open-rows">${all.map(r=>`<div class="open-row"><span class="dot"></span><strong>${esc(users.get(r.user_id)?.display_name||'Onbekend')}</strong>${r.user_id===pd.host_id?'<span class="mini-pill">Host</span>':''}</div>`).join('')||'<div class="empty-state">Nog niemand aangemeld.</div>'}</div></section>
  ...
  function renderRanking(){
    ...
    const me=current(),mine=statsFor(me.id),rank=ordered.findIndex(r=>r.id===me.id)+1;
    return `${uiHeader('Statistieken','Jouw prestaties en de volledige competitie in één overzicht.')}
      <div class="stats-compact-grid ranking-summary flat-kpis"><section class="stat-compact"><span>Ranking</span><strong>#${rank||'–'}</strong></section><section class="stat-compact"><span>Punten</span><strong>${mine.points}</strong></section><section class="stat-compact"><span>Wedstrijden</span><strong>${mine.played}</strong></section><section class="stat-compact"><span>Winst %</span><strong>${mine.winPct}%</strong></section></div>
      <div class="personal-detail-strip"><span><small>Winst-verlies</small><b>${mine.wins}-${mine.losses}</b></span><span><small>Sets</small><b>${mine.setsWon}-${mine.setsLost}</b></span><span><small>Games</small><b>${mine.gamesWon}-${mine.gamesLost}</b></span><span><small>Saldo</small><b>${mine.gameDiff>0?'+':''}${mine.gameDiff}</b></span></div>
      <section class="flat-section"><div class="section-title"><h2>Competitieranglijst</h2><span class="muted small-note">Tik op een speler</span></div><div class="competition-list">${rows.map((r,i)=>`<button class="competition-row ${r.id===me.id?'me':''}" data-player="${r.id}"><span class="rank-index">${i+1}</span><span class="player-cell"><strong>${esc(r.name)}</strong><small>${r.wins} winst · ${r.losses} verlies · saldo ${r.gameDiff>0?'+':''}${r.gameDiff}</small><i class="form-line">${r.recent.map(x=>`<em class="${x==='W'?'win':x==='V'?'loss':'draw'}">${x}</em>`).join('')||'<em>–</em>'}</i></span><span class="row-stat"><b>${r.points}</b><small>pt</small></span><span class="row-stat"><b>${r.played}</b><small>wed.</small></span><span class="row-stat wide"><b>${r.setsWon}-${r.setsLost}</b><small>sets</small></span><span class="row-stat wide"><b>${r.gamesWon}-${r.gamesLost}</b><small>games</small></span><span class="chev">›</span></button>`).join('')||'<div class="empty-state">Nog geen spelers.</div>'}</div></section>
      <section class="flat-section scoring-note"><h2>Puntentelling</h2><p class="muted">Elke gewonnen game is 1 punt. De winnaar van een volledig uitgespeelde wedstrijd krijgt 3 bonuspunten.</p></section>${poweredFooter()}`;
  }
  function renderHistory(){
    ...
    return `${uiHeader('Historie','Alleen jouw eigen gespeelde speeldagen, wedstrijden en resultaten.')}
      ...
      <div class="personal-history-list">${days.map(({pd,ms})=>{let wins=0,pts=0;ms.forEach(m=>{const blue=m.blue_player_1===me.id||m.blue_player_2===me.id;pts+=S.awardedPoints(m,blue?'blue':'red');if(m.set_completed&&m.winner_team===(blue?'blue':'red'))wins++;});return `<button class="personal-day-row" data-history-playday="${pd.id}">${fmtDayBadge(pd.date)}<span class="personal-day-main"><strong>${esc(pd.location||'Locatie volgt')}</strong><small>${fmtTime(pd.start_time)} - ${fmtTime(pd.end_time)} · ${ms.length} wedstrijd${ms.length===1?'':'en'}</small><span>${wins} gewonnen · ${ms.length-wins} niet gewonnen</span></span><span class="personal-day-points"><b>+${pts}</b><small>punten</small></span><span class="chev">›</span></button>`}).join('')||'<div class="empty-state">Je hebt nog geen afgeronde wedstrijden gespeeld.</div>'}</div>${poweredFooter()}`;
  }
  function renderAccount(){
    if(isAdmin())return renderAdmin();
    const me=current();
    return `${uiHeader('Account','Je persoonlijke gegevens en beveiliging.')}
      <section class="account-compact"><div class="account-avatar">${esc(me.display_name.slice(0,1).toUpperCase())}</div><div><strong>${esc(me.display_name)}</strong><span>@${esc(me.username)} · speler</span></div></section>
      <div class="account-actions"><button class="open-action" data-change-password><span>Wachtwoord wijzigen</span><b>›</b></button><button class="open-action danger-text" data-logout><span>Uitloggen</span><b>›</b></button></div>${poweredFooter()}`;
  }
  function renderAdmin(){
    if(!isAdmin())return '<section class="card empty-state">Geen toegang.</section>';
    const users=DB.listUsers(),pending=users.filter(u=>u.approval_status==='pending'),players=users.filter(u=>u.approval_status!=='pending'&&u.approval_status!=='rejected'),pds=DB.listPlaydays().sort((a,b)=>a.date.localeCompare(b.date)||a.start_time.localeCompare(b.start_time));
    return `${uiHeader('Beheer','Compact beheer van aanmeldingen, spelers en speeldagen.')}
      ...
      ${state.adminTab==='requests'?`<section class="flat-section admin-panel"><div class="section-title"><div><h2>Nieuwe aanmeldingen</h2><p class="muted">Controleer wie toegang krijgt.</p></div></div><div class="compact-admin-list">${pending.map(u=>`<article class="compact-admin-row"><div class="admin-avatar">${esc(u.display_name.slice(0,1).toUpperCase())}</div><div><strong>${esc(u.display_name)}</strong><small>@${esc(u.username)} · wacht op goedkeuring</small></div><div class="admin-item-actions"><button class="btn small primary" data-approve-user="${u.id}">Goedkeuren</button><button class="btn small danger" data-reject-user="${u.id}">Afwijzen</button></div></article>`).join('')||'<div class="empty-state">Geen openstaande aanmeldingen.</div>'}</div></section>`:state.adminTab==='players'?`<section class="flat-section admin-panel"><div class="section-title admin-title"><div><h2>Spelers (${players.filter(u=>u.role==='player'&&u.active).length})</h2><p class="muted">Accounts wijzigen, resetten of blokkeren.</p></div><button class="btn primary small" data-add-user>+ Speler</button></div><div class="compact-admin-list">${players.map(u=>`<article class="compact-admin-row"><div class="admin-avatar">${esc(u.display_name.slice(0,1).toUpperCase())}</div><div><strong>${esc(u.display_name)} ${u.role==='admin'?'<span class="badge yellow">Beheerder</span>':''}</strong><small>@${esc(u.username)} · ${u.active?'actief':'geblokkeerd'}</small></div><div class="admin-item-actions">${u.role!=='admin'?`<button class="btn small ghost" data-edit-user="${u.id}">Bewerk</button><button class="btn small ghost" data-reset-user="${u.id}">Reset</button><button class="btn small danger" data-delete-user="${u.id}">${u.active?'Blokkeer':'Geblokkeerd'}</button>`:''}</div></article>`).join('')}</div></section>`:state.adminTab==='playdays'?`<section class="flat-section admin-panel"><div class="section-title admin-title"><div><h2>Speeldagen</h2><p class="muted">Toevoegen, wijzigen of verwijderen.</p></div><button class="btn primary small" data-add-pd>+ Speeldag</button></div><div class="compact-admin-list">${pds.map(p=>`<article class="compact-admin-row playday-admin-row"><div>${fmtDayBadge(p.date)}</div><div><strong>${esc(p.location||'Locatie volgt')}</strong><small>${fmtTime(p.start_time)} – ${fmtTime(p.end_time)} · host ${esc(nameOf(p.host_id))}</small></div><div class="admin-item-actions"><button class="btn small ghost" data-open-playday="${p.id}">Open</button><button class="btn small ghost" data-edit-pd-id="${p.id}">Wijzig</button><button class="btn small danger" data-delete-pd-id="${p.id}">Verwijder</button></div></article>`).join('')||'<div class="empty-state">Geen speeldagen.</div>'}</div></section>`:`<section class="flat-section admin-panel"><section class="account-compact"><div class="account-avatar">${esc(current().display_name.slice(0,1).toUpperCase())}</div><div><strong>${esc(current().display_name)}</strong><span>@${esc(current().username)} · beheerder</span></div></section><form id="registrationCodeForm" class="inline-setting"><div><strong>Competitiecode wijzigen</strong><small>Minimaal 6 tekens. Deel deze alleen met spelers.</small></div><input name="competition_code" minlength="6" placeholder="Nieuwe competitiecode" required><button class="btn primary small">Opslaan</button></form><div class="account-actions"><button class="open-action" data-change-password><span>Wachtwoord wijzigen</span><b>›</b></button><button class="open-action danger-text" data-logout><span>Uitloggen</span><b>›</b></button></div></section>`}${poweredFooter()}`;
  }
  function bindPage(){
    ...
    $$('[data-playday-view]').forEach(b=>b.onclick=()=>{state.playdayView=b.dataset.playdayView;render();});
    $$('[data-playday-filter]').forEach(b=>b.onclick=()=>{state.playdayFilter=b.dataset.playdayFilter;render();});
  ...
  function openPlayerHistory(id){
    ...
    modal(`Historie van ${nameOf(id)}`,`<div class="modal-open-list">${days.map(({pd,ms})=>`<button class="open-action" data-modal-history="${pd.id}"><span><strong>${esc(fmtDate(pd.date))}</strong><small>${esc(pd.location||'Locatie volgt')} · ${ms.length} wedstrijd${ms.length===1?'':'en'}</small></span><b>›</b></button>`).join('')||'<div class="empty-state">Nog geen historie.</div>'}</div>`,()=>{$$('[data-modal-history]').forEach(b=>b.onclick=()=>openPersonalHistory(b.dataset.modalHistory,id));});
  }
  ...
  async function boot(){ bindGlobal(); if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./service-worker.js?v=3.2.0',{updateViaCache:'none'});await reg.update();if(reg.waiting)reg.waiting.postMessage('SKIP_WAITING');navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!sessionStorage.getItem('wepadel-sw-320')){sessionStorage.setItem('wepadel-sw-320','1');location.reload();}});}catch{}} try{await DB.init();}catch(e){toast(e.message||'Online verbinding mislukt.',true);} if(current())showApp();else showLogin(); }
  boot();
})();
