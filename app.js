(() => {
  ...
  function renderDashboard(){
    ...
    return `<div class="dashboard-greeting"><h1>Goedemiddag</h1><strong>${esc(me.display_name)}</strong></div>
  ...
  function renderMonth(year,month,byDate){
    ...
    for(let d=1;d<=days;d++){
      ...
      const cls=['day',date===todayISO()?'today':'',date===state.selectedCalendarDate?'selected':'',arr.length?'playday':''].filter(Boolean).join(' ');
  ...
  function renderAccount(){
    ...
    return `${uiHeader('Account','')}
      ...
      <div class="account-actions"><button class="open-action" data-change-password><span>Wachtwoord wijzigen</span><b>›</b></button><button class="open-action danger-text" data-logout><span>Uitloggen</span><b>›</b></button></div>`;
  }
  ...
  async function boot(){ bindGlobal(); if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./service-worker.js?v=3.3.0',{updateViaCache:'none'});await reg.update();if(reg.waiting)reg.waiting.postMessage('SKIP_WAITING');navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!sessionStorage.getItem('wepadel-sw-330')){sessionStorage.setItem('wepadel-sw-330','1');location.reload();}});}catch{}} try{await DB.init();}catch(e){toast(e.message||'Online verbinding mislukt.',true);} if(current())showApp();else showLogin(); }
  boot();
})();
